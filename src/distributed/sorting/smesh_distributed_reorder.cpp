...
#define SMESH_EXPLICIT_INSTANTIATE_DISTRIBUTED_REORDER(IDX_T, GEOM_T)          \
  ...
  template int distributed_assign_elements_sfc_multiblock<                     \
      ...
      const ptrdiff_t *const SMESH_RESTRICT,                                   \
      const ptrdiff_t *const SMESH_RESTRICT,                                   \
...
#define SMESH_EXPLICIT_INSTANTIATE_MESH_FROM_FOLDER_REORDERED(                 \
  ...
  template int mesh_from_folder_reordered<                                     \
      IDX_T, GEOM_T, LARGE_IDX_T, OrderEncoder<GEOM_T>>(                        \
      ...
      OrderEncoder<GEOM_T> ordering);                                          \
  template int mesh_from_folder_multiblock<                                    \
      IDX_T, GEOM_T, LARGE_IDX_T, OrderEncoder<GEOM_T>>(                       \
      ...
      ptrdiff_t **node_offsets_out, IDX_T **ghosts_out, const bool use_sfc,    \
      OrderEncoder<GEOM_T> ordering)
...
SMESH_EXPLICIT_INSTANTIATE_MESH_FROM_FOLDER_REORDERED(i32, f64, i32);
SMESH_EXPLICIT_INSTANTIATE_MESH_FROM_FOLDER_REORDERED(i32, f64, i64);
SMESH_EXPLICIT_INSTANTIATE_MESH_FROM_FOLDER_REORDERED(i64, f64, i64);

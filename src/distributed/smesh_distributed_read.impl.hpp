...
int read_mapped_field(MPI_Comm comm, const char *input_path,
  ...
  SMESH_MPI_CATCH(MPI_Alltoall(req_count, 1, smesh::mpi_type<int>(), recv_req_count, 1,
                               smesh::mpi_type<int>(), comm));
  ...
  ptrdiff_t *book_keeping = (ptrdiff_t *)calloc((size_t)size, sizeof(ptrdiff_t));
  ...
  SMESH_MPI_CATCH(MPI_Alltoallv(req_list, req_count, req_displs, smesh::mpi_type<idx_t>(),
                                recv_req_list, recv_req_count, recv_displs,

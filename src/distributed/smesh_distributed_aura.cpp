...
#define SMESH_INSTANTIATE_EXCHANGE_CREATE(IDX_T)                               \
  ...
  template int exchange_create_ghosts<IDX_T>(                                  \
      MPI_Comm comm, const ptrdiff_t n_local_nodes,                            \
      const ptrdiff_t n_owned_nodes,                                           \
      const int *const SMESH_RESTRICT node_owner,                              \
      const ptrdiff_t *const SMESH_RESTRICT node_offsets,                      \
...
#define SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_FOR_IDX(IDX_T)                  \
  ...
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, f16);                   \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, i32);                          \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, i32);                   \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, i8);                           \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, i8);                    \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, char);                         \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, char);                  \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, i64);                          \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, i64);                   \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, i16);                          \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, i16);                   \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, f32);                          \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, f32);                   \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, f64);                          \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, f64);                   \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, u8);                           \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, u8);                    \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, u16);                          \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, u16);                   \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, u32);                          \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, u32);                   \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD(IDX_T, u64);                          \
  SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(IDX_T, u64)
...
SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(i32, long);
SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS(i64, long);
#endif

} // namespace smesh

#undef SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_FOR_IDX
#undef SMESH_INSTANTIATE_EXCHANGE_SCATTER_ADD_GHOSTS

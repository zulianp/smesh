...
namespace smesh {
    ...
    // SharedBuffer<jacobian_t *> KernelData::jacobians_SoA(const block_idx_t block_id) {
    //     SMESH_ASSERT(impl_->jacobians_SoA.size() > block_id);
    //     return impl_->jacobians_SoA[block_id];
    // }

    // SharedBuffer<jacobian_t> KernelData::jacobians_AoS(const block_idx_t block_id) {
    //     SMESH_ASSERT(impl_->jacobians_AoS.size() > block_id);
    //     return impl_->jacobians_AoS[block_id];
    // }

    // SharedBuffer<jacobian_t *> KernelData::jacobian_adjugate_SoA(const block_idx_t block_id) {
    //     SMESH_ASSERT(impl_->jacobian_adjugate_SoA.size() > block_id);
    //     return impl_->jacobian_adjugate_SoA[block_id];
    // }

    // SharedBuffer<jacobian_t> KernelData::jacobian_adjugate_AoS(const block_idx_t block_id) {
    //     SMESH_ASSERT(impl_->jacobian_adjugate_AoS.size() > block_id);
    //     return impl_->jacobian_adjugate_AoS[block_id];
    // }

    // SharedBuffer<geom_t> KernelData::jacobian_determinant(const block_idx_t block_id) {
    ...
    // void KernelData::set_elements_SoA(const block_idx_t block_id, const SharedBuffer<idx_t *> &elements) {
    //     SMESH_ASSERT(impl_->elements_SoA.size() > block_id);
    //     impl_->elements_SoA[block_id] = elements;
    // }
    // void KernelData::set_elements_AoS(const block_idx_t block_id, const SharedBuffer<idx_t> &elements) {
    //     SMESH_ASSERT(impl_->elements_AoS.size() > block_id);
    //     impl_->elements_AoS[block_id] = elements;
    // }
    // void KernelData::set_points_SoA(const SharedBuffer<geom_t *> &points) { impl_->points_SoA = points; }
    // void KernelData::set_points_AoS(const SharedBuffer<geom_t> &points) { impl_->points_AoS = points; }
    // void KernelData::set_jacobians_SoA(const block_idx_t block_id, const SharedBuffer<jacobian_t *> &jacobians) {
    //     SMESH_ASSERT(impl_->jacobians_SoA.size() > block_id);
    //     impl_->jacobians_SoA[block_id] = jacobians;
    // }
    // void KernelData::set_jacobians_AoS(const block_idx_t block_id, const SharedBuffer<jacobian_t> &jacobians) {
    //     SMESH_ASSERT(impl_->jacobians_AoS.size() > block_id);
    //     impl_->jacobians_AoS[block_id] = jacobians;
    // }
    // void KernelData::set_jacobian_adjugate_SoA(const block_idx_t block_id, const SharedBuffer<jacobian_t *> &jacobian_adjugate)
    // {
    //     SMESH_ASSERT(impl_->jacobian_adjugate_SoA.size() > block_id);
    //     impl_->jacobian_adjugate_SoA[block_id] = jacobian_adjugate;
    // }
    // void KernelData::set_jacobian_adjugate_AoS(const block_idx_t block_id, const SharedBuffer<jacobian_t> &jacobian_adjugate) {
    //     SMESH_ASSERT(impl_->jacobian_adjugate_AoS.size() > block_id);
    //     impl_->jacobian_adjugate_AoS[block_id] = jacobian_adjugate;
    // }
    // void KernelData::set_jacobian_determinant(const block_idx_t block_id, const SharedBuffer<geom_t> &jacobian_determinant) {
    //     SMESH_ASSERT(impl_->jacobian_determinant.size() > block_id);
    ...
    //             impl_->jacobian_adjugate_SoA[i] = to_device(impl_->jacobian_adjugate_SoA[i]);
    //         }
    //     }
    //     for (int i = 0; i < n_blocks; ++i) {
    //         if (impl_->jacobian_adjugate_AoS[i]) {
    //             impl_->jacobian_adjugate_AoS[i] = to_device(impl_->jacobian_adjugate_AoS[i]);
    //         }
    //     }
    //     for (int i = 0; i < n_blocks; ++i) {
    //         if (impl_->jacobian_determinant[i]) {
    //             impl_->jacobian_determinant[i] = to_device(impl_->jacobian_determinant[i]);
    //         }
    //     }
    ...
    //     std::shared_ptr<KernelData> Mesh::create_kernel_data(const int flags, const enum ExecutionSpace space) {
    ...
    //                 auto block = blocks[i];
    //                 if (block && block->elements()) {
    //                     auto elements = soa_to_aos(block->n_nodes_per_element(), block->n_elements(), block->elements());
    ...
    //             geo_data->set_points_AoS(soa_to_aos(points->extent(0), points->extent(1), points));
    //         }
    ...
    //                 auto block = blocks[i];
    //                 if (block) {
    //                     auto adjugate = create_host_buffer<jacobian_t>(block->n_nodes_per_element(), block->n_elements());
    ...
    //                 auto block = blocks[i];
    //                 if (block) {
    //                     auto adjugate = create_host_buffer<jacobian_t>(block->n_elements() * block->n_nodes_per_element());

    //                     auto fake_adjugate = convert_host_buffer_to_fake_SoA(block->n_nodes_per_element(), adjugate);
    ...
    int Mesh::write(const Path &path) const {
        ...
        if (impl_->comm->size() == 1) {
            ...
            if (impl_->blocks.size() != 1 || !impl_->blocks[0]) {
                SMESH_ERROR(
                        "Mesh::write (MPI) currently supports only single-block "
                        "meshes.\n");
            }
            ...
            int err = write_distributed_mesh_topology(impl_->comm->get(),
                                                      path,
                                                      et,
                                                      this->spatial_dimension(),
    ...
    std::shared_ptr<Mesh::NodeToNodeGraph> Mesh::create_node_to_node_graph(const enum ElemType element_type) {
        if (n_blocks() != 1) {
            SMESH_ERROR(
                    "create_node_to_node_graph is not supported for multi-block "
                    "meshes!\n");
            return nullptr;
        }
        ...
        const ptrdiff_t n_nodes = max_node_id(element_type, n_elements(0), elements(0)->data()) + 1;
        ...
        if (is_semistructured_type(this->element_type(0))) {
            ...
            sshex8_crs_graph<element_idx_t, count_t, idx_t>(proteus_hex_micro_elements_per_dim(element_type),
                                                            this->n_elements(0),
                                                            this->n_nodes(),
            ...
            create_crs_graph_for_elem_type(element_type, n_elements(0), n_nodes, elements(0)->data(), &rowptr, &colidx);
        }

        auto crs_graph =
                std::make_shared<Mesh::NodeToNodeGraph>(Buffer<count_t>::own(n_nodes + 1, rowptr, free, MEMORY_SPACE_HOST),
    ...
    std::shared_ptr<Mesh> Mesh::create_semistructured_hex_cube(const std::shared_ptr<Communicator> &comm,
        ...
        const ptrdiff_t nnodes = (nx * micro_nodes_per_dim) * (ny * micro_nodes_per_dim) * (nz * micro_nodes_per_dim);
...
#pragma omp parallel for
    ...
    int Mesh::split_boundary_layer() {
...

        {
            ...
            if (extract_skin_sideset(this->n_elements(0),
                                     this->n_nodes(),
                                     this->element_type(0),
                                     this->elements(0)->data(),
                                     &n_surf_elements,
            ...
            sideset = std::make_shared<Sideset>(
                    this->comm(), manage_host_buffer(n_surf_elements, parent), manage_host_buffer(n_surf_elements, side_idx));
        }
    ...
    std::shared_ptr<Mesh> promote_to(const enum ElemType element_type, const std::shared_ptr<Mesh> &mesh) {
        ...
        cmap[std::make_pair(TRI3, TRI6)] = [](Mesh &mesh) {
            ...
            auto points   = create_host_buffer<geom_t>(mesh.spatial_dimension(), n2n_upper_triangular->colidx()->size());
        ...
        if (mesh->comm()->size() > 1) {
...

            {
                ...
                for (i64 i = 0; i < recv_displs[comm_size]; ++i) {
                    const ptrdiff_t local_element = owned_global_to_local[recv_global_ids[i] - element_start];
                    if (local_element < n_owned_not_shared || local_element >= n_owned_elements) {
                        continue;
                    }
    ...
    std::shared_ptr<Mesh> extrude(const std::shared_ptr<Mesh> &mesh, const geom_t height, const ptrdiff_t nlayers) {
        ...
        if (mesh->n_nodes_per_element(0) == 4) {
            auto hex8_elements = create_host_buffer<idx_t>(8, mesh->n_elements(0) * nlayers);

            auto hex8_points = create_host_buffer<geom_t>(3, mesh->n_nodes() * (nlayers + 1));

            quad4_to_hex8_extrude(mesh->n_elements(0),
                                  mesh->n_nodes(),
                                  mesh->elements(0)->data(),
                                  mesh->points()->data(),
                                  nlayers,
            ...
            auto wedge6_elements = create_host_buffer<idx_t>(6, mesh->n_elements(0) * nlayers);

            auto wedge6_points = create_host_buffer<geom_t>(3, mesh->n_nodes() * (nlayers + 1));

            tri3_to_wedge6_extrude(mesh->n_elements(0),
                                   mesh->n_nodes(),
                                   mesh->elements(0)->data(),
                                   mesh->points()->data(),
                                   nlayers,
            ...
            return std::make_shared<Mesh>(mesh->comm(), WEDGE6, wedge6_elements, wedge6_points);
        } else {

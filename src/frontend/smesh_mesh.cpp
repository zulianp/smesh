...
std::shared_ptr<Sideset> skin_sideset(const std::shared_ptr<Mesh> &mesh) {
  ...
  if (is_semistructured_type(mesh->element_type(0)) &&
      semistructured_level(mesh->element_type(0)) > 1) {
  ...
  if (mesh->comm()->size() > 1) {
    ...
    if (n_owned_elements > 0) {
      owned_global_to_local = (element_idx_t *)SMESH_ALLOC(
          (size_t)n_owned_elements * sizeof(element_idx_t));
...

    {
      ...
      i64 *send_displs =
          (i64 *)SMESH_CALLOC((size_t)comm_size + 1, sizeof(i64));

...
SharedBuffer<void> to_device<void>(const SharedBuffer<void> &in) {
    ...
    device::host_to_device<char>(nbytes, static_cast<const char *>(in->void_data()), buff);
...
SharedBuffer<void *> to_device<void>(const SharedBuffer<void *> &in) {
    ...
    auto dev_buff0 = reinterpret_cast<void **>(device::alloc<char>(n0 * sizeof(void *)));
    ...
    device::host_to_device<char>(n0 * sizeof(void *), reinterpret_cast<const char *>(host_dev_ptrs.data()),

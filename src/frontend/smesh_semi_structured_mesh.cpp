...
namespace smesh {
    ...
    std::shared_ptr<SemiStructuredMesh> SemiStructuredMesh::derefine(const int to_level) {
        ...
        if (this->impl_->points) {
            ...
            ret->impl_->points = smesh::view(this->impl_->points, 0, sdim, 0, n_unique_nodes);
        }
    ...
    int SemiStructuredMesh::apply_hierarchical_renumbering() {
        if (n_blocks() > 1) {
            SMESH_ERROR("Applying hierarchical renumbering is not supported for multi-block meshes");
        }
    ...
    std::shared_ptr<Buffer<geom_t *>> SemiStructuredMesh::points() {
        if (!impl_->points) {
            auto p       = smesh::create_host_buffer<geom_t>(impl_->macro_mesh->spatial_dimension(), impl_->n_unique_nodes);
    ...
    int SemiStructuredMesh::export_as_standard(const Path &folder) {
        ...
        std::stringstream ss;
        ss << "# SMESH mesh meta file (generated by SemiStructuredMesh::export_as_standard)\n";

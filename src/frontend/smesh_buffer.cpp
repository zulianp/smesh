...
namespace smesh {
    ...
    BaseBuffer::BaseBuffer(const size_t                n,
...
#define SMESH_EXPLICIT_INSTANTIATE_BUFFER_1D(T)                                   \
    ...
    template std::shared_ptr<Buffer<T>> manage_host_buffer<T>(const size_t, T *); \
    template std::shared_ptr<Buffer<T>> view<T>(const std::shared_ptr<Buffer<T>> &, const size_t, const size_t)
...
#define SMESH_EXPLICIT_INSTANTIATE_BUFFER_2D(T)                                                                                 \
    ...
    template std::shared_ptr<Buffer<T *>> create_host_buffer<T>(const size_t, const size_t);                                    \
    template std::shared_ptr<Buffer<T *>> create_host_buffer_fake_SoA<T>(const size_t, const size_t);                           \
    template std::shared_ptr<Buffer<T *>> convert_host_buffer_to_fake_SoA<T>(const size_t, const std::shared_ptr<Buffer<T>> &); \
    template std::shared_ptr<Buffer<T *>> manage_host_buffer<T>(const size_t, const size_t, T **);                              \
    template std::shared_ptr<Buffer<T *>> view<T>(                                                                              \
            const std::shared_ptr<Buffer<T *>> &, const size_t, const size_t, const size_t, const size_t);                      \
    template std::shared_ptr<Buffer<T>>   sub<T>(const std::shared_ptr<Buffer<T *>> &, const size_t);                           \
    template std::shared_ptr<Buffer<T *>> copy<T>(const std::shared_ptr<Buffer<T *>> &);                                        \
    ...
    SMESH_EXPLICIT_INSTANTIATE_BUFFER_2D(f64);
...
#define SMESH_EXPLICIT_INSTANTIATE_BUFFER_ASTYPE(O, I)                                      \
    ...
    SMESH_EXPLICIT_INSTANTIATE_BUFFER_ASTYPE(f32, f64);

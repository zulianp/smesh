...
template <typename T> class Restrict<T>::Impl {
public:
      ...
      if (is_semistructured_type(from_element)) {
        if (!is_semistructured_type(to_element)) {
          actual_op = [=](const real_t *const from, real_t *const to) -> int {
            ...
            return sshex8_hierarchical_restriction(
                semistructured_level(*from_mesh), from_mesh->n_elements(),
                from_mesh->elements(0)->data(),
                element_to_node_incidence_count->data(), block_size, from, to);
          };
          return;
        } else {
          actual_op = [=](const real_t *const from, real_t *const to) -> int {
            ...
            return sshex8_restrict(
                from_mesh->n_elements(),          // nelements,
        ...
        actual_op = [=](const real_t *const from, real_t *const to) -> int {
          ...
          return hierarchical_restriction(
              from_element, to_element, from_mesh->n_elements(),
              from_mesh->elements(0)->data(),
              element_to_node_incidence_count->data(), block_size, from, to);
        };
        return;
  ...
  Impl(const std::shared_ptr<Mesh> &from, const std::shared_ptr<Mesh> &to,
...
Restrict<T>::Restrict(const std::shared_ptr<Mesh> &from,
                      const std::shared_ptr<Mesh> &to, const ExecutionSpace es,
                      const int block_size)
    : impl_(std::make_unique<Impl>(from, to, es, block_size)) {}

template <typename T>
std::shared_ptr<Restrict<T>>
Restrict<T>::create(const std::shared_ptr<Mesh> &from,
                    const std::shared_ptr<Mesh> &to, const ExecutionSpace es,
                    const int block_size) {
  return std::make_shared<Restrict<T>>(from, to, es, block_size);
}

template <typename T> Restrict<T>::~Restrict() = default;

template <typename T> int Restrict<T>::apply(const T *const x, T *const y) {
  return impl_->apply(x, y);
}

template <typename T> ptrdiff_t Restrict<T>::rows() const {
  return impl_->from_mesh->n_nodes() * impl_->block_size;
}

template <typename T> ptrdiff_t Restrict<T>::cols() const {
  return impl_->to_mesh->n_nodes() * impl_->block_size;
}

template <typename T> ExecutionSpace Restrict<T>::execution_space() const {
  return impl_->es;
}

template <typename T>
const SharedBuffer<uint16_t> &
Restrict<T>::element_to_node_incidence_count() const {
  return impl_->element_to_node_incidence_count;
}

template <typename T> class SurfaceRestrict<T>::Impl {
public:
  int from_level;
  ...
  Impl(const int from_level, const ElemType from_elem_type,
       ...
       const ElemType to_elem_type, const ptrdiff_t to_n_nodes,
       const SharedBuffer<idx_t *> &to_sides, const ExecutionSpace es,
       const int block_size)
      : from_level(from_level), from_elem_type(from_elem_type),
        from_n_nodes(from_n_nodes), from_sides(from_sides),
        from_count(from_count), to_level(to_level), to_elem_type(to_elem_type),
        to_n_nodes(to_n_nodes), to_sides(to_sides), es(es),
...
#endif
    ssquad4_restrict(from_sides->extent(1), from_level, 1, from_sides->data(),
                     from_count->data(), to_level, 1, to_sides->data(),
                     block_size, x, y);
...
template <typename T>
SurfaceRestrict<T>::SurfaceRestrict(
    const int from_level, const ElemType from_elem_type,
    const ptrdiff_t from_n_nodes, const SharedBuffer<idx_t *> &from_sides,
    const SharedBuffer<uint16_t> &from_count, const int to_level,
    const ElemType to_elem_type, const ptrdiff_t to_n_nodes,
    const SharedBuffer<idx_t *> &to_sides, const ExecutionSpace es,
    ...
    : impl_(std::make_unique<Impl>(
          from_level, from_elem_type, from_n_nodes, from_sides, from_count,
          to_level, to_elem_type, to_n_nodes, to_sides, es, block_size)) {}

template <typename T>
std::shared_ptr<SurfaceRestrict<T>> SurfaceRestrict<T>::create(
    const int from_level, const ElemType from_elem_type,
    const ptrdiff_t from_n_nodes, const SharedBuffer<idx_t *> &from_sides,
    const SharedBuffer<uint16_t> &from_count, const int to_level,
    const ElemType to_elem_type, const ptrdiff_t to_n_nodes,
    const SharedBuffer<idx_t *> &to_sides, const ExecutionSpace es,
    const int block_size) {
  return std::make_shared<SurfaceRestrict<T>>(
      from_level, from_elem_type, from_n_nodes, from_sides, from_count,
      to_level, to_elem_type, to_n_nodes, to_sides, es, block_size);
}

template <typename T> SurfaceRestrict<T>::~SurfaceRestrict() = default;
...
template <typename T> ptrdiff_t SurfaceRestrict<T>::rows() const {
  return impl_->to_n_nodes * impl_->block_size;
}

template <typename T> ptrdiff_t SurfaceRestrict<T>::cols() const {

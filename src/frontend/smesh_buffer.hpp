...
namespace smesh {
    ...
    public:
        ...
        static std::shared_ptr<Buffer<T>> wrap(const size_t n, T *const x, enum MemorySpace mem_space = MEMORY_SPACE_INVALID);

        static std::shared_ptr<Buffer<T>> own(const size_t                n,
                                              T                          *x,
                                              std::function<void(void *)> destroy,
    ...
    public:
        ...
        static std::shared_ptr<Buffer<T *>> wrap(const size_t     n0,
                                                 const size_t     n1,
                                                 T              **x,
                                                 enum MemorySpace mem_space = MEMORY_SPACE_INVALID);

        static std::shared_ptr<Buffer<T *>> own(const size_t                        n0,
                                                const size_t                        n1,
                                                T                                 **x,
                                                std::function<void(int n, void **)> destroy,
                                                enum MemorySpace                    mem_space = MEMORY_SPACE_INVALID);
...
namespace smesh {
    ...
    template <typename T>
    void check_NaNs(const SharedBuffer<T> &buffer, const char *filename, int line) {
        auto host_buffer = to_host(buffer);
        for (size_t i = 0; i < host_buffer->size(); i++) {
            if (std::isnan(host_buffer->data()[i])) {
                SMESH_ERROR("NaN found in buffer at index %zu in %s:%d", i, filename, line);
            }
        }
    }

#define SMESH_CHECK_NANS(buffer)                \
    do {                                        \
        check_NaNs(buffer, __FILE__, __LINE__); \
    } while (0)
    //
}  // namespace smesh

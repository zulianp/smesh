...
#else
  auto ret = std::make_shared<Buffer<T>>(n, (T *)device::alloc<T>(n),
                                         &device::destroy, MEMORY_SPACE_DEVICE);
  return ret;
...
template <typename T> SharedBuffer<T> to_host(const SharedBuffer<T> &in) {
  ...
  NonConstT *buff =
      static_cast<NonConstT *>(SMESH_ALLOC(in->size() * sizeof(NonConstT)));
  SMESH_ASSERT(buff != nullptr);
  device::device_to_host(in->size(), in->data(), buff);
  return std::make_shared<Buffer<T>>(in->size(), buff, &free,
                                     MEMORY_SPACE_HOST);
#endif // SMESH_ENABLE_CUDA
}

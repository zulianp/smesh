...
namespace smesh {
    ...
    public:
        ...
        friend std::shared_ptr<Mesh> mesh_from_sideset_parallel(const std::shared_ptr<Mesh>    &mesh,
                                                                const std::shared_ptr<Sideset> &sideset);
    ...
    //     GEO_ALL_SOA = GEO_ELEMENT_SOA | GEO_POINT_SOA | GEO_JACOBIAN_SOA | GEO_JACOBIAN_ADJUGATE_SOA |
    //     GEO_JACOBIAN_DETERMINANT, GEO_ALL_AOS = GEO_ELEMENT_AOS | GEO_POINT_AOS | GEO_JACOBIAN_AOS | GEO_JACOBIAN_ADJUGATE_AOS,
    //     GEO_ALL     = GEO_ALL_SOA | GEO_ALL_AOS
    // };
    ...
    //     void set_elements_SoA(const block_idx_t block_id, const SharedBuffer<idx_t *> &elements);
    //     void set_elements_AoS(const block_idx_t block_id, const SharedBuffer<idx_t> &elements);
    //     void set_points_SoA(const SharedBuffer<geom_t *> &points);
    //     void set_points_AoS(const SharedBuffer<geom_t> &points);
    //     void set_jacobians_SoA(const block_idx_t block_id, const SharedBuffer<jacobian_t *> &jacobians);
    //     void set_jacobians_AoS(const block_idx_t block_id, const SharedBuffer<jacobian_t> &jacobians);
    //     void set_jacobian_adjugate_SoA(const block_idx_t block_id, const SharedBuffer<jacobian_t *> &jacobian_adjugate);
    //     void set_jacobian_adjugate_AoS(const block_idx_t block_id, const SharedBuffer<jacobian_t> &jacobian_adjugate);
    //     void set_jacobian_determinant(const block_idx_t block_id, const SharedBuffer<geom_t> &jacobian_determinant);
    ...
    public:
        ...
        // std::shared_ptr<KernelData> create_kernel_data(const int flags, const enum ExecutionSpace space);

        // int                         init_kernel_data(const int flags, const enum ExecutionSpace space);
        // std::shared_ptr<KernelData> kernel_data() const;
        ...
        static std::shared_ptr<Mesh> create_semistructured_hex_cube(const std::shared_ptr<Communicator> &comm,
                                                                    const int       micro_elements_per_dim = 2,
                                                                    const ptrdiff_t nx                     = 1,
                                                                    const ptrdiff_t ny                     = 1,
                                                                    const ptrdiff_t nz                     = 1,
                                                                    const geom_t    xmin                   = 0,
        ...
        void reorder_elements_from_tags(const block_idx_t block_id, const SharedBuffer<idx_t> &tags);

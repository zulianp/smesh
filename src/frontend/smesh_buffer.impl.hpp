...
namespace smesh {
    ...
    template <typename T>
    Buffer<T>::Buffer(const size_t n, T *const ptr, std::function<void(void *)> destroy, MemorySpace mem_space)
        : BaseBuffer(n, TypeToEnum<T>::value(), (void *)ptr, destroy, mem_space) {}

    template <typename T>
    Buffer<T>::Buffer() : BaseBuffer(0, SMESH_TYPE_UNDEFINED, nullptr, nullptr, MEMORY_SPACE_INVALID) {}
    ...
    template <typename T>
    void Buffer<T>::print(std::ostream &os) const {
    ...
    template <typename T>
    std::shared_ptr<Buffer<T>> Buffer<T>::wrap(const size_t n, T *const x, enum MemorySpace mem_space) {
        return std::make_shared<Buffer<T>>(n, x, nullptr, mem_space);
    }

    template <typename T>
    std::shared_ptr<Buffer<T>> Buffer<T>::own(const size_t                n,
                                              T                          *x,
    ...
    template <typename T>
    std::shared_ptr<Buffer<T>> Buffer<T>::make_empty() {
        return std::make_shared<Buffer<T>>();
    }

    template <typename T>
    int Buffer<T>::to_file(const Path &path) const {
        return array_write_convert_from_extension<typename std::remove_const_t<T>>(path, data(), this->n_);
    }

    template <typename T>
    std::shared_ptr<Buffer<T>> Buffer<T>::from_file(const Path &path) {
        if constexpr (std::is_const_v<T>) {
            SMESH_ERROR("Cannot read const buffer from file\n");
            return nullptr;
        }
    ...
    template <typename T>
    void Buffer<T *>::print(std::ostream &os) {
    ...
    template <typename T>
    std::shared_ptr<Buffer<T *>> Buffer<T *>::wrap(const size_t n0, const size_t n1, T **x, enum MemorySpace mem_space) {
        return std::make_shared<Buffer<T *>>(n0, n1, x, nullptr, mem_space);
    }

    template <typename T>
    std::shared_ptr<Buffer<T *>> Buffer<T *>::own(const size_t                        n0,
                                                  const size_t                        n1,
                                                  T                                 **x,
    ...
    template <typename T>
    int Buffer<T *>::to_files(const Path &format) {
        ...
        for (int i = 0; i < static_cast<int>(extent_[0]); i++) {
            ...
            if (array_write_convert_from_extension<typename std::remove_const_t<T>>(Path(path), ptr_[i], extent_[1])) {
                ret = SMESH_FAILURE;
            }
        }
    ...
    template <typename T>
    std::shared_ptr<Buffer<T *>> convert_host_buffer_to_fake_SoA(const size_t n0, const std::shared_ptr<Buffer<T>> &in) {
        assert(n0 > 0);
        assert(in->size() % n0 == 0);
    ...
    template <typename T>
    std::shared_ptr<Buffer<T *>> manage_host_buffer(const size_t n0, const size_t n1, T **data) {
        auto ret = std::make_shared<Buffer<T *>>(
                n0,
                n1,
                data,
    ...
    template <typename T>
    std::shared_ptr<Buffer<T>> view(const std::shared_ptr<Buffer<T>> &buffer, const size_t begin, const size_t end) {
        return std::make_shared<Buffer<T>>(
                end - begin,
                const_cast<T *>(&buffer->data()[begin]),
                [keep_alive = buffer](void *) { (void)keep_alive; },
                buffer->mem_space());
    }

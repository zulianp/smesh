...
namespace smesh {
    ...
    MPI_Datatype Grid<T>::mpi_data_type() const {
        ...
        SMESH_ERROR("Unsupported type request in Grid::mpi_data_type()\n");
    ...
    std::shared_ptr<Grid<T>> Grid<T>::create_from_file(const std::shared_ptr<Communicator> &comm, const std::string &path) {
...
#if defined(SMESH_ENABLE_RYAML)
        {
            ...
            if (!require("path", field_path) || !require("spatial_dimension", ret->impl_->spatial_dimension) ||
                !require("nx", ret->impl_->nglobal[0]) || !require("ox", ret->impl_->origin[0]) || !require("dx", ret->impl_->delta[0])) {
                SMESH_ERROR("Grid::create_from_file: Missing required metadata in %s\n", (path + "/meta.yaml").c_str());
                return nullptr;
            }

            if (ret->impl_->spatial_dimension > 1 &&
                (!require("ny", ret->impl_->nglobal[1]) || !require("oy", ret->impl_->origin[1]) || !require("dy", ret->impl_->delta[1]))) {
                SMESH_ERROR("Grid::create_from_file: Missing required metadata in %s\n", (path + "/meta.yaml").c_str());
                return nullptr;
            }

            if (ret->impl_->spatial_dimension > 2 &&
                (!require("nz", ret->impl_->nglobal[2]) || !require("oz", ret->impl_->origin[2]) || !require("dz", ret->impl_->delta[2]))) {
                SMESH_ERROR("Grid::create_from_file: Missing required metadata in %s\n", (path + "/meta.yaml").c_str());
                return nullptr;
            }
...
#else
        {
            ...
            auto read_scalar = [](const std::string &line, const char *key, auto &value) -> bool {
            ...
            while (std::getline(ifs, line)) {
                if (line.rfind("path:", 0) == 0) {
                    ...
                    field_path = first == std::string::npos ? std::string() : field_path.substr(first);
                    continue;
                }

                read_scalar(line, "spatial_dimension:", ret->impl_->spatial_dimension) ||
                    read_scalar(line, "nx:", ret->impl_->nglobal[0]) || read_scalar(line, "ox:", ret->impl_->origin[0]) ||
                    read_scalar(line, "dx:", ret->impl_->delta[0]) || read_scalar(line, "ny:", ret->impl_->nglobal[1]) ||
                    read_scalar(line, "oy:", ret->impl_->origin[1]) || read_scalar(line, "dy:", ret->impl_->delta[1]) ||
                    read_scalar(line, "nz:", ret->impl_->nglobal[2]) || read_scalar(line, "oz:", ret->impl_->origin[2]) ||
                    read_scalar(line, "dz:", ret->impl_->delta[2]) || read_scalar(line, "block_size:", ret->impl_->block_size);
            }
...
#ifdef SMESH_ENABLE_MPI
        {
            ...
            if (ret->impl_->spatial_dimension > 1) size *= ret->impl_->nlocal[1];
            if (ret->impl_->spatial_dimension > 2) size *= ret->impl_->nlocal[2];

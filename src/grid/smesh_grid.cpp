...
namespace smesh {
    ...
    void create_xdmf(const Grid<T> &grid, const Path &path_binary_field, std::ostream &os) {
        ...
        os << "<!DOCTYPE Xdmf SYSTEM \"Xdmf.dtd\" []>\n"
           ...
           << n[2] << ' ' << n[1] << ' ' << n[0] << ' ' << block_size << "\" Endian=\"" << endianess << "\" Precision=\""
           << sizeof(T) << "\" NumberType=\"" << number_type
           << "\">\n"
              "                    "
           << path_binary_field
           << "\n"
    ...
    std::shared_ptr<Grid<T>> Grid<T>::create_from_file(const std::shared_ptr<Communicator> &comm, const std::string &path) {
...
#if defined(SMESH_ENABLE_RYAML)
        {
            ...
            if (!require("path", field_path) || !require("spatial_dimension", ret->impl_->spatial_dimension) ||
                !require("nx", ret->impl_->nglobal[0]) || !require("ox", ret->impl_->origin[0]) ||
            ...
            if (ret->impl_->spatial_dimension > 1 &&
                (!require("ny", ret->impl_->nglobal[1]) || !require("oy", ret->impl_->origin[1]) ||
            ...
            if (ret->impl_->spatial_dimension > 2 &&
                (!require("nz", ret->impl_->nglobal[2]) || !require("oz", ret->impl_->origin[2]) ||
...
#pragma omp parallel for
    ...
    template <class T>
    int Grid<T>::to_file(const Path &folder) {
        std::stringstream ss;

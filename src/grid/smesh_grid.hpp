...
template <class T> using SharedGrid = std::shared_ptr<Grid<T>>;
...
public:
  ...
  static std::shared_ptr<Grid> create(const std::shared_ptr<Communicator> &comm,
                                      ...
                                      const ptrdiff_t nz, const geom_t xmin,
                                      const geom_t ymin, const geom_t zmin,
...
template <typename T> SharedGrid<T> to_device(const SharedGrid<T> &grid) {
#ifdef SMESH_ENABLE_CUDA

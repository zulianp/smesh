...
namespace smesh {
    ...
    int create_n2e(const ptrdiff_t                                         nelements,
...
#endif
        element_idx_t *elindex = (element_idx_t *)SMESH_ALLOC(static_cast<size_t>(n2eptr[nnodes]) * sizeof(element_idx_t));
        ...
        for (int edof_i = 0; edof_i < nnodesxelem; ++edof_i) {
            for (ptrdiff_t i = 0; i < nelements; ++i) {
                ...
                elindex[n2eptr[node] + book_keeping[node]++] = static_cast<element_idx_t>(i);
            }
        }
    ...
    int sort_n2e(const ptrdiff_t                     nnodes,
...
#pragma omp parallel for
    ...
    template <typename idx_t, typename count_t, typename element_idx_t>
    static int create_n2e_for_elem_type(const enum ElemType                                     element_type,
    ...
    template <typename idx_t, typename count_t, typename element_idx_t>
    int create_n2n_from_n2e(const ptrdiff_t                                         nelements,
                            const ptrdiff_t                                         nnodes,
                            const int                                               nnodesxelem,
                            const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                            const count_t *const SMESH_RESTRICT                     n2eptr,
...
#pragma omp for
    ...
    static int create_crs_graph_mem_conservative(const ptrdiff_t                                         nelements,
                                                 const ptrdiff_t                                         nnodes,
                                                 const int                                               nnodesxelem,
                                                 const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                                                 count_t                                               **out_rowptr,
        ...
        create_n2e<idx_t, count_t, element_idx_t_local>(nelements, nnodes, nnodesxelem, elems, &n2eptr, &elindex);
    ...
    template <typename idx_t, typename count_t>
    static int create_crs_graph_faster(const ptrdiff_t                                         nelements,
                                       const ptrdiff_t                                         nnodes,
                                       const int                                               nnodesxelem,
                                       const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                                       count_t                                               **out_rowptr,
...
#pragma omp parallel for reduction(+ : overestimated_nnz)
    ...
    template <typename idx_t, typename count_t>
    int create_crs_graph_for_elem_type(const enum ElemType                                     element_type,
                                       const ptrdiff_t                                         nelements,
                                       const ptrdiff_t                                         nnodes,
                                       const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                                       count_t                                               **out_rowptr,
    ...
    template <typename idx_t, typename count_t>
    int create_crs_graph_from_element(const ptrdiff_t                                         nelements,
                                      const ptrdiff_t                                         nnodes,
                                      int                                                     nxe,
                                      const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                                      count_t                                               **out_rowptr,
        ...
        if (SMESH_CRS_FAST_SERIAL) {
            return create_crs_graph_faster<idx_t, count_t>(nelements, nnodes, nxe, elems, out_rowptr, out_colidx);
        }

        return create_crs_graph_mem_conservative<idx_t, count_t>(nelements, nnodes, nxe, elems, out_rowptr, out_colidx);
    }

    template <typename idx_t, typename count_t>
    int create_crs_graph(const ptrdiff_t                                         nelements,
                         const ptrdiff_t                                         nnodes,
                         const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                         count_t                                               **out_rowptr,
                         idx_t                                                 **out_colidx) {
        return create_crs_graph_for_elem_type<idx_t, count_t>(TET4, nelements, nnodes, elems, out_rowptr, out_colidx);
    }

    template <typename idx_t, typename count_t>
    int create_crs_graph_3(const ptrdiff_t                                         nelements,
                           const ptrdiff_t                                         nnodes,
                           const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                           count_t                                               **out_rowptr,
                           idx_t                                                 **out_colidx) {
        return create_crs_graph_for_elem_type<idx_t, count_t>(TRI3, nelements, nnodes, elems, out_rowptr, out_colidx);
    }
    ...
    template <typename idx_t, typename count_t, typename element_idx_t>
    static int create_dual_graph_from_n2e(const ptrdiff_t                                         n_elements,
        ...
        element_idx_t *dual_eidx = (element_idx_t *)SMESH_CALLOC(
                static_cast<size_t>(n_overestimated_connections + extra_buffer_space), sizeof(element_idx_t));
            ...
            for (int en = 0; en < n_nodes_per_elem; en++) {
                ...
                for (count_t eii = n2eptr[node]; eii < n2eptr[node + 1]; eii++) {
                    ...
                    if (connection_counter[e_adj] == 0) {
                        const ptrdiff_t write_pos = static_cast<ptrdiff_t>(offset) + static_cast<ptrdiff_t>(count_common);
    ...
    int create_dual_graph(const ptrdiff_t                                         n_elements,
        ...
        if (element_type == TET10) {
            create_n2e<idx_t, count_t, element_idx_t>(n_elements, n_nodes, elem_num_nodes(TET4), elems, &n2eptr, &elindex);
        } else {
            create_n2e<idx_t, count_t, element_idx_t>(
                    n_elements, n_nodes, elem_num_nodes(element_type), elems, &n2eptr, &elindex);
        }

        const int ret = create_dual_graph_from_n2e<idx_t, count_t, element_idx_t>(
                n_elements, n_nodes, element_type, elems, n2eptr, elindex, out_rowptr, out_colidx);
    ...
    template <typename idx_t, typename count_t, typename element_idx_t>
    static int create_crs_graph_upper_triangular_from_n2e(const ptrdiff_t                                         nelements,
                                                          const ptrdiff_t                                         nnodes,
                                                          const int                                               nnodesxelem,
                                                          const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                                                          const count_t *const SMESH_RESTRICT                     n2eptr,
...
#pragma omp parallel for
    ...
    template <typename idx_t, typename count_t>
    int create_crs_graph_upper_triangular_from_element(const ptrdiff_t                                         nelements,
                                                       const ptrdiff_t                                         nnodes,
                                                       int                                                     nxe,
                                                       const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                                                       count_t                                               **out_rowptr,
        ...
        count_t             *n2eptr;
        element_idx_t_local *elindex;
        create_n2e<idx_t, count_t, element_idx_t_local>(nelements, nnodes, nxe, elems, &n2eptr, &elindex);

        int err = create_crs_graph_upper_triangular_from_n2e<idx_t, count_t, element_idx_t_local>(
    ...
    template <typename idx_t, typename count_t>
    int crs_to_coo(const ptrdiff_t n, const count_t *const SMESH_RESTRICT rowptr, idx_t *const SMESH_RESTRICT row_idx) {
#pragma omp parallel for

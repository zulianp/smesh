...
static int create_dual_graph_from_n2e(
    ...
    const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
    const count_t *const SMESH_RESTRICT n2eptr,
    const element_idx_t *const SMESH_RESTRICT elindex, count_t **out_dual_eptr,
...
#endif
  count_t *dual_e_ptr =
      (count_t *)SMESH_CALLOC((n_elements + 1), sizeof(count_t));
...
template <typename idx_t, typename count_t>
int create_edge_graph_for_element_from_n2e(
    const enum ElemType element_type, const ptrdiff_t nelements,
    const ptrdiff_t nnodes,
    const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
    const count_t *const SMESH_RESTRICT n2eptr,
  ...
  if (element_type == TET4 || element_type == TRI3 ||
    ...
    return create_crs_graph_upper_triangular_from_n2e<idx_t, count_t>(
        nelements, nnodes, elem_num_nodes(element_type), elems, n2eptr, elindex,
        out_rowptr, out_colidx);
  }

  if (element_type == QUAD4 || element_type == QUADSHELL4) {
    return quad4_build_edge_graph_from_n2e<idx_t, count_t, element_idx_t>(
        nelements, nnodes, elems, n2eptr, elindex, out_rowptr, out_colidx);
  }

  SMESH_ERROR("create_edge_graph_for_element: unsupported element type: %s",

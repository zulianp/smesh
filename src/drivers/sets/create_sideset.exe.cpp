...
int main(int argc, char **argv) {
    ...
    {
        const geom_t roi[3] = {(geom_t)std::atof(argv[2]), (geom_t)std::atof(argv[3]), (geom_t)std::atof(argv[4])};
        ...
        auto selected = create_host_buffer<mask_t>(mask_count(sideset->parent()->size()));

        const int err = sideset_select_propagate(sideset->parent()->size(),
                                                 sideset->parent()->data(),
                                                 sideset->lfi()->data(),
                                                 n2e->rowptr()->data(),
                                                 n2e->colidx()->data(),
                                                 ...
                                                 [&](const ptrdiff_t prev_side, const ptrdiff_t next_side) {
                                                     ...
                                                     real_t n0[3] = {0, 0, 0};
                                                     real_t n1[3] = {0, 0, 0};
                                                     if (!compute_side_normal(dim, element_type, elements, points, pe, ps, n0)) {
                                                         return false;
                                                     }
                                                     if (!compute_side_normal(dim, element_type, elements, points, ne, ns, n1)) {
                                                         return false;
                                                     }
        ...
        auto out_ss = Sideset::create(mesh->comm(), out_parent, out_lfi, sideset->block_id());

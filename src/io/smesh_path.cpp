...
Path::Path(std::string_view path) : path_(path) {
...
#ifdef WIN32
  static const char tobereplaced = '/';
  static const char replacement = '\\';
#else
  static const char tobereplaced = '\\';
  static const char replacement = '/';
#endif
  std::replace(path.begin(), path.end(), tobereplaced, replacement);
...
Path Path::operator/(const char *other) const { return (*this) / Path(other); }
...
PathIterator::DirHandle::DirHandle(const std::string_view &path) {
...
PathIterator::PathIterator(const Path &path)
    : path_(path), dir_(new DirHandle(path.to_string())), it_(nullptr),
      skip_hidden_(true) {
...
Path Path::absolute() const {
  return Path(std::filesystem::absolute(path_).string());
}
} // namespace smesh

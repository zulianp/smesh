...
namespace smesh {
int array_write(const Path &path, const enum PrimitiveType type,
...
int mesh_write_yaml_basic(const Path &path, enum ElemType element_type,
  ...
  for (int d = 0; d < spatial_dim; d++) {
    fprintf(meta_file, "- %c: %c.%s\n", xyz[d], xyz[d],
            TypeToString<geom_t>::value().data());
  }
...
SMESH_EXPLICIT_INSTANTIATE_ARRAY_WRITE(mask_t);
SMESH_EXPLICIT_INSTANTIATE_ARRAY_WRITE(ptrdiff_t);
...
SMESH_EXPLICIT_INSTANTIATE_ARRAY_WRITE_CONVERT_FROM_EXTENSION(mask_t);
SMESH_EXPLICIT_INSTANTIATE_ARRAY_WRITE_CONVERT_FROM_EXTENSION(ptrdiff_t);

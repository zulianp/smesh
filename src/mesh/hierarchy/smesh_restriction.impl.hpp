...
#pragma omp parallel for
            for (ptrdiff_t e = 0; e < nelements; e++) {
                ...
                for (int v = 0; v < vec_size; v++) {
                    ...
                    const T to0 = from[ii0] / e2n_count[i0] + from[ii4] * (0.5 / e2n_count[i4]) +
                                  from[ii6] * (0.5 / e2n_count[i6]) + from[ii7] * (0.5 / e2n_count[i7]);

                    const T to1 = from[ii1] / e2n_count[i1] + from[ii5] * (0.5 / e2n_count[i5]) +
                                  from[ii8] * (0.5 / e2n_count[i8]) + from[ii4] * (0.5 / e2n_count[i4]);

                    const T to2 = from[ii2] / e2n_count[i2] + from[ii9] * (0.5 / e2n_count[i9]) +
                                  from[ii5] * (0.5 / e2n_count[i5]) + from[ii6] * (0.5 / e2n_count[i6]);

                    const T to3 = from[ii3] / e2n_count[i3] + from[ii7] * (0.5 / e2n_count[i7]) +
                                  from[ii8] * (0.5 / e2n_count[i8]) + from[ii9] * (0.5 / e2n_count[i9]);
...
#pragma omp parallel for
            for (ptrdiff_t e = 0; e < nelements; e++) {
                ...
                for (int v = 0; v < vec_size; v++) {
                    ...
                    const T to0 =
                            from[ii0] / e2n_count[i0] + from[ii3] * (0.5 / e2n_count[i3]) + from[ii5] * (0.5 / e2n_count[i5]);
                    const T to1 =
                            from[ii1] / e2n_count[i1] + from[ii3] * (0.5 / e2n_count[i3]) + from[ii4] * (0.5 / e2n_count[i4]);
                    const T to2 =
                            from[ii2] / e2n_count[i2] + from[ii4] * (0.5 / e2n_count[i4]) + from[ii5] * (0.5 / e2n_count[i5]);
...
#pragma omp atomic update
            ...
            SMESH_ERROR("Unsupported element pair for hierarchical_restriction %d, %d\n", from_element, to_element);
    ...
    int hierarchical_restriction(
        ...
        for (ptrdiff_t i = 0; i < nnodes; i++) {
            ...
            for (int k = 0; k < extent; k++) {
                ...
                if (i < j) {
                    for (int v = 0; v < vec_size; v++) {
                        const T edge_value = from[fine_idx * vec_size + v];

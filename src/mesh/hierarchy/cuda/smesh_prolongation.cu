...
__global__ void cu_tet4_to_macrotet4_prolongation_kernel(
    ...
    const idx_t *const SFEM_RESTRICT fine_node_map, const int vec_size,
    const From *const SFEM_RESTRICT from, To *const SFEM_RESTRICT to) {
  for (ptrdiff_t i = blockIdx.x * blockDim.x + threadIdx.x; i < coarse_nnodes;
    ...
    for (int k = 0; k < extent; k++) {
      ...
      if (i < j) {
        SMESH_ASSERT(edge >= coarse_nnodes);
...
template <typename From, typename To, typename idx_t>
static int cu_tet4_to_macrotet4_prolongation_tpl(
    const ptrdiff_t coarse_nnodes,
    const count_t *const SFEM_RESTRICT coarse_rowptr,
    const idx_t *const SFEM_RESTRICT coarse_colidx,
    const idx_t *const SFEM_RESTRICT fine_node_map, const int vec_size,
...
#ifdef SFEM_USE_OCCUPANCY_MAX_POTENTIAL
  {
    ...
    cudaOccupancyMaxPotentialBlockSize(
        ...
        cu_tet4_to_macrotet4_prolongation_kernel<From, To, idx_t>, 0, 0);
  }
#endif // SFEM_USE_OCCUPANCY_MAX_POTENTIAL

  ptrdiff_t n_blocks =
      MAX(ptrdiff_t(1), (coarse_nnodes + block_size - 1) / block_size);

  if (stream) {
    cudaStream_t s = *static_cast<cudaStream_t *>(stream);

    cu_tet4_to_macrotet4_prolongation_kernel<From, To, idx_t>
        <<<n_blocks, block_size, 0, s>>>(coarse_nnodes, coarse_rowptr,
                                         coarse_colidx, fine_node_map, vec_size,
                                         from, to);
  } else {
    cu_tet4_to_macrotet4_prolongation_kernel<From, To, idx_t>
        <<<n_blocks, block_size, 0>>>(coarse_nnodes, coarse_rowptr,
                                      coarse_colidx, fine_node_map, vec_size,
                                      from, to);
  }
...
int cu_tet4_to_macrotet4_prolongation(
    ...
    const idx_t *const SFEM_RESTRICT coarse_colidx,
    const idx_t *const SFEM_RESTRICT fine_node_map, const int vec_size,
    const enum RealType from_type, const void *const SFEM_RESTRICT from,
    const enum RealType to_type, void *const SFEM_RESTRICT to, void *stream) {
  ...
  default: {
    SFEM_ERROR("[Error] cu_tet4_to_macrotet4_prolongation_tpl: not implemented "
               "for type %s "
               "(code %d)\n",
...
__global__ void cu_macrotet4_to_tet4_prolongation_elemental_kernel(
    const ptrdiff_t nelements, const idx_t *const SFEM_RESTRICT*const SFEM_RESTRICT elements,
    const int vec_size, const ptrdiff_t from_stride,
    const From *const SFEM_RESTRICT from, const ptrdiff_t to_stride,
  ...
  for (ptrdiff_t e = blockIdx.x * blockDim.x + threadIdx.x; e < nelements;
    ...
    for (int v = 0; v < vec_size; v++) {
      ...
      to[ii4 * to_stride] =
          0.5 * (from[ii0 * from_stride] + from[ii1 * from_stride]);
      to[ii5 * to_stride] =
          0.5 * (from[ii1 * from_stride] + from[ii2 * from_stride]);
      to[ii6 * to_stride] =
          0.5 * (from[ii0 * from_stride] + from[ii2 * from_stride]);
      to[ii7 * to_stride] =
          0.5 * (from[ii0 * from_stride] + from[ii3 * from_stride]);
      to[ii8 * to_stride] =
          0.5 * (from[ii1 * from_stride] + from[ii3 * from_stride]);
      to[ii9 * to_stride] =
          0.5 * (from[ii2 * from_stride] + from[ii3 * from_stride]);
    }
  }
}
...
static int cu_macrotet4_to_tet4_prolongation_elemental_tpl(
    ...
    const int vec_size, const ptrdiff_t from_stride,
    const From *const SFEM_RESTRICT from, const ptrdiff_t to_stride,
    To *const SFEM_RESTRICT to, void *stream) {
  // Hand tuned
  int block_size = 128;
#ifdef SFEM_USE_OCCUPANCY_MAX_POTENTIAL
  {
  ...
  if (stream) {
    ...
    cu_macrotet4_to_tet4_prolongation_elemental_kernel<From, To, idx_t>
        <<<n_blocks, block_size, 0, s>>>(nelements, elements, vec_size,
                                         from_stride, from, to_stride, to);
  } else {
    cu_macrotet4_to_tet4_prolongation_elemental_kernel<From, To, idx_t>
        <<<n_blocks, block_size, 0>>>(nelements, elements, vec_size,
                                      from_stride, from, to_stride, to);
  }
...
int cu_macrotet4_to_tet4_prolongation_elemental(
    ...
    const ptrdiff_t from_stride, const void *const SFEM_RESTRICT from,
    const enum RealType to_type, const ptrdiff_t to_stride,
    void *const SFEM_RESTRICT to, void *stream) {
  SMESH_ASSERT(from_type == to_type && "TODO mixed types!");
  if (from_type != to_type) {
    return SFEM_FAILURE;
  }
  ...
  case SFEM_FLOAT32: {
    return cu_macrotet4_to_tet4_prolongation_elemental_tpl(
        nelements, elements, vec_size, from_stride, (f32 *)from, to_stride,
        (f32 *)to, stream);
  }
  case SFEM_FLOAT64: {
    return cu_macrotet4_to_tet4_prolongation_elemental_tpl(
        nelements, elements, vec_size, from_stride, (f64 *)from, to_stride,
        (f64 *)to, stream);
  }
  default: {
    SFEM_ERROR("[Error] cu_tet4_to_macrotet4_prolongation_tpl: not implemented "

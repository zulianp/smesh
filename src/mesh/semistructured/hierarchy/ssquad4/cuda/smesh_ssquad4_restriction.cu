    ...
    for (int d = 0; d < vec_size; d++) {
      ...
      for (int v = 0; v < 4; v++) {
        const ptrdiff_t global_to_idx =
            (elements[corners[v] * stride + e] * vec_size + d) * to_stride;
...
template <typename From, typename To>
static int cu_ssquad4_hierarchical_restriction_tpl(
    const int level, const ptrdiff_t nelements, const ptrdiff_t stride,
    const idx_t *const SMESH_RESTRICT elements,
...
#ifdef SMESH_USE_OCCUPANCY_MAX_POTENTIAL
  {
    ...
    cudaOccupancyMaxPotentialBlockSize(
        ...
        cu_ssquad4_hierarchical_restriction_kernel<From, To>, 0, 0);
  }
  ...
  if (stream) {
    ...
    cu_ssquad4_hierarchical_restriction_kernel<From, To>
...
extern int cu_ssquad4_hierarchical_restriction(
  ...
  case SMESH_FLOAT32: {
    return cu_ssquad4_hierarchical_restriction_tpl(
        level, nelements, stride, elements, element_to_node_incidence_count,
        vec_size, from_stride, (float *)from, to_stride, (float *)to, stream);
  }
  case SMESH_FLOAT64: {
    return cu_ssquad4_hierarchical_restriction_tpl(
        level, nelements, stride, elements, element_to_node_incidence_count,
        vec_size, from_stride, (double *)from, to_stride, (double *)to, stream);
  }
  default: {
    SMESH_ERROR(
...
template <typename From, typename To>
__global__ void cu_ssquad4_hierarchical_restriction_SoA_kernel(
...
template <typename From, typename To>
static int cu_ssquad4_hierarchical_restriction_SoA_tpl(
    const int level, const ptrdiff_t nelements,
    idx_t **const SMESH_RESTRICT elements,
    const uint16_t *const SMESH_RESTRICT element_to_node_incidence_count,
    const int vec_size, const ptrdiff_t from_stride,
...
#ifdef SMESH_USE_OCCUPANCY_MAX_POTENTIAL
  {
    ...
    cudaOccupancyMaxPotentialBlockSize(
        &min_grid_size, &block_size,
        cu_ssquad4_hierarchical_restriction_SoA_kernel<From, To>, 0, 0);
  }
#endif // SMESH_USE_OCCUPANCY_MAX_POTENTIAL
  ...
  if (stream) {
    ...
    cu_ssquad4_hierarchical_restriction_SoA_kernel<From, To>
        <<<n_blocks, block_size, 0, s>>>(
...
template <typename From, typename To>
__global__ void cu_ssquad4_restrict_kernel(
    ...
    idx_t **const SMESH_RESTRICT to_elements, const To *const SMESH_RESTRICT S,
    const int vec_size, const ptrdiff_t from_stride,
    const From *const SMESH_RESTRICT from, const ptrdiff_t to_stride,
    To *const SMESH_RESTRICT to) {
  static_assert(TILE_SIZE == 4,
                "This only works with tile size 4 because the implementation "
                "assumes a fixed tile size for shared memory "
                "layout and indexing.");
        ...
        for (int from_yi = y_odd; from_yi < y_end; from_yi += 2) {
          for (int from_xi = x_odd; from_xi < x_end; from_xi += 2) {
            // Parallel read from global mem on 4 fine nodes
            {
              ...
              if (from_exists) {
                const int idx_from =
                    cu_ssquad4_lidx(from_level * from_level_stride,
...
int cu_ssquad4_restrict_tpl(
...
#ifdef SMESH_USE_OCCUPANCY_MAX_POTENTIAL
  {
    ...
    cudaOccupancyMaxPotentialBlockSize(&min_grid_size, &block_size,
                                       cu_ssquad4_restrict_kernel<From, To>, 0,
                                       0);
  }
#endif // SMESH_USE_OCCUPANCY_MAX_POTENTIAL
...
extern int cu_ssquad4_restrict(
  ...
  case SMESH_REAL_DEFAULT: {
    return cu_ssquad4_restrict_tpl<real_t, real_t>(
        ...
        from_level, from_level_stride, from_elements,
        from_element_to_node_incidence_count, to_level, to_level_stride,
        to_elements, vec_size, from_stride, (real_t *)from, to_stride,
        (real_t *)to, stream);
  }
  case SMESH_FLOAT32: {
    return cu_ssquad4_restrict_tpl<float, float>(
        nelements,
        // stride,
        from_level, from_level_stride, from_elements,
        from_element_to_node_incidence_count, to_level, to_level_stride,
        to_elements, vec_size, from_stride, (float *)from, to_stride,
        (float *)to, stream);
  }
  case SMESH_FLOAT64: {
    return cu_ssquad4_restrict_tpl<double, double>(
        nelements,
        // stride,
        from_level, from_level_stride, from_elements,
        from_element_to_node_incidence_count, to_level, to_level_stride,

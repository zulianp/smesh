...
int test_trace_space_operations(
    const std::shared_ptr<Mesh> &coarse_mesh,
    const std::shared_ptr<Mesh> &fine_mesh,
    const std::vector<std::shared_ptr<Sideset>> &sideset,
    const std::string & /*name*/, const int block_size,
  ...
  auto coarse_x =
      create_host_buffer<real_t>(coarse_mesh->n_nodes() * block_size);
  auto fine_x = create_buffer<real_t>(fine_mesh->n_nodes() * block_size, es);

  ptrdiff_t n_nodes{0};
  idx_t *nodes{nullptr};
  SMESH_TEST_ASSERT(smesh::sshex8_extract_nodeset_from_sideset(
                        smesh::semistructured_level(*coarse_mesh),
                        coarse_mesh->elements(0)->data(),
                        sideset[0]->parent()->size(),
                        sideset[0]->parent()->data(), sideset[0]->lfi()->data(),
                        &n_nodes, &nodes) == SMESH_SUCCESS);
  ...
  auto fine_sides =
      create_host_buffer<idx_t>(nexs, sideset[0]->parent()->size());

  SMESH_TEST_ASSERT(smesh::sshex8_extract_surface_from_sideset(
                        fine_level, fine_mesh->elements(0)->data(),
                        sideset[0]->parent()->size(),
                        sideset[0]->parent()->data(), sideset[0]->lfi()->data(),
  ...
  SMESH_TEST_ASSERT(ssquad4_element_node_incidence_count(
                        fine_level, 1, fine_sides->extent(1),
                        fine_sides->data(), count->data()) == SMESH_SUCCESS);

  SMESH_TEST_ASSERT(
      ssquad4_restrict(fine_sides->extent(1), fine_level, 1, fine_sides->data(),
                       count->data(), coarse_level, fine_level / coarse_level,
                       fine_sides->data(), block_size, fine_x->data(),
...
int test_trace_space_prolongation_restriction() {
  ...
  geom_t Lx = 1;
  auto m = Mesh::create_hex8_cube(comm, SMESH_BASE_RESOLUTION * 1,
                                  SMESH_BASE_RESOLUTION * 1,
                                  SMESH_BASE_RESOLUTION * 1, 0, 0, 0, Lx, 1, 1);
  ...
  auto sideset = Sideset::create_from_selector(
      m, [=](const geom_t /*x*/, const geom_t y, const geom_t /*z*/) -> bool {
        return y > -1e-5 && y < 1e-5;
      });

  return test_trace_space_operations(
      derefine(m, 2), m, {sideset}, "test_trace_space_prolongation_restriction",
      block_size, es);
}

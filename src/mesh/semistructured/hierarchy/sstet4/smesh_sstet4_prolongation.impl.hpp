#ifndef SMESH_SSTET4_PROLONGATION_IMPL_HPP
#define SMESH_SSTET4_PROLONGATION_IMPL_HPP

#include "smesh_sstet4_prolongation.hpp"
#include "smesh_alloc.hpp"
#include "smesh_sstet4_transfer.impl.hpp"

namespace smesh {

    template <typename idx_t, typename T>
    int sstet4_hierarchical_prolongation(int                                                     level,
                                         const ptrdiff_t                                         nelements,
                                         const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elements,
                                         const int                                               vec_size,
                                         const T *const SMESH_RESTRICT                           from,
                                         T *const SMESH_RESTRICT                                 to) {
        const int corners[4] = {sstet4_lidx(level, 0, 0, 0),
                                sstet4_lidx(level, level, 0, 0),
                                sstet4_lidx(level, 0, level, 0),
                                sstet4_lidx(level, 0, 0, level)};

#pragma omp parallel for schedule(static)
        for (ptrdiff_t e = 0; e < nelements; ++e) {
            for (int z = 0; z <= level; ++z) {
                for (int y = 0; y <= level - z; ++y) {
                    for (int x = 0; x <= level - z - y; ++x) {
                        const int lidx = sstet4_lidx(level, x, y, z);
                        const T   h    = T(1) / T(level);
                        const T   w[4] = {T(1) - T(x + y + z) * h, T(x) * h, T(y) * h, T(z) * h};
                        const idx_t gid = elements[lidx][e];

                        for (int d = 0; d < vec_size; ++d) {
                            T val = 0;
                            for (int v = 0; v < 4; ++v) {
                                val += w[v] * from[elements[corners[v]][e] * vec_size + d];
                            }
                            to[gid * vec_size + d] = val;
                        }
                    }
                }
            }
        }

        return SMESH_SUCCESS;
    }

    template <typename idx_t, typename T>
    int sstet4_prolongate(const ptrdiff_t                                         nelements,
                          const int                                               from_level,
                          const int                                               from_level_stride,
                          const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT from_elements,
                          const int                                               to_level,
                          const int                                               to_level_stride,
                          const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT to_elements,
                          const int                                               vec_size,
                          const T *const SMESH_RESTRICT                           from,
                          T *const SMESH_RESTRICT                                 to) {
        SMESH_ASSERT(to_level % from_level == 0);
        if (to_level % from_level != 0) {
            SMESH_ERROR("Nested meshes requirement: to_level must be divisible by from_level!");
            return SMESH_FAILURE;
        }

        const int from_nxe = sstet4_nxe(from_level);
        int       invalid  = 0;

#pragma omp parallel reduction(| : invalid)
        {
            int *const cx = (int *)SMESH_ALLOC(static_cast<size_t>(from_nxe) * sizeof(int));
            int *const cy = (int *)SMESH_ALLOC(static_cast<size_t>(from_nxe) * sizeof(int));
            int *const cz = (int *)SMESH_ALLOC(static_cast<size_t>(from_nxe) * sizeof(int));
            sstet4_transfer::fill_lattice_coords(from_level, cx, cy, cz);

#pragma omp for schedule(static)
            for (ptrdiff_t e = 0; e < nelements; ++e) {
                for (int z = 0; z <= to_level; ++z) {
                    for (int y = 0; y <= to_level - z; ++y) {
                        for (int x = 0; x <= to_level - z - y; ++x) {
                            int coarse_lidx[4];
                            T   w[4];
                            const int err = sstet4_transfer::find_weights(from_level,
                                                                          T(x) * T(from_level) / T(to_level),
                                                                          T(y) * T(from_level) / T(to_level),
                                                                          T(z) * T(from_level) / T(to_level),
                                                                          cx,
                                                                          cy,
                                                                          cz,
                                                                          coarse_lidx,
                                                                          w);
                            if (err != SMESH_SUCCESS) {
                                invalid = 1;
                                continue;
                            }

                            const int to_lidx =
                                    sstet4_lidx(to_level * to_level_stride, x * to_level_stride, y * to_level_stride, z * to_level_stride);
                            const idx_t to_gid = to_elements[to_lidx][e];

                            for (int d = 0; d < vec_size; ++d) {
                                T val = 0;
                                for (int v = 0; v < 4; ++v) {
                                    const int from_lidx = sstet4_lidx(from_level * from_level_stride,
                                                                      cx[coarse_lidx[v]] * from_level_stride,
                                                                      cy[coarse_lidx[v]] * from_level_stride,
                                                                      cz[coarse_lidx[v]] * from_level_stride);
                                    val += w[v] * from[from_elements[from_lidx][e] * vec_size + d];
                                }
                                to[to_gid * vec_size + d] = val;
                            }
                        }
                    }
                }
            }

            SMESH_FREE(cx);
            SMESH_FREE(cy);
            SMESH_FREE(cz);
        }

        return invalid ? SMESH_FAILURE : SMESH_SUCCESS;
    }

}  // namespace smesh

#endif  // SMESH_SSTET4_PROLONGATION_IMPL_HPP

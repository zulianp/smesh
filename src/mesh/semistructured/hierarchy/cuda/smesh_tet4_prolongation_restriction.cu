...
__global__ void cu_tet4_to_macrotet4_prolongation_kernel(
    ...
    const From *const SMESH_RESTRICT from, To *const SMESH_RESTRICT to) {
...
static int cu_tet4_to_macrotet4_prolongation_tpl(
...
#ifdef SMESH_USE_OCCUPANCY_MAX_POTENTIAL
  {
    ...
    cudaOccupancyMaxPotentialBlockSize(
        ...
        cu_tet4_to_macrotet4_prolongation_kernel<From, To>, 0, 0);
  }
...
extern int cu_tet4_to_macrotet4_prolongation(
    const ptrdiff_t coarse_nnodes,
    const count_t *const SMESH_RESTRICT coarse_rowptr,
    const idx_t *const SMESH_RESTRICT coarse_colidx,
    const idx_t *const SMESH_RESTRICT fine_node_map, const int vec_size,
  ...
  case SMESH_FLOAT32: {
    return cu_tet4_to_macrotet4_prolongation_tpl(
        coarse_nnodes, coarse_rowptr, coarse_colidx, fine_node_map, vec_size,
        (float *)from, (float *)to, stream);
  }
  case SMESH_FLOAT64: {
    return cu_tet4_to_macrotet4_prolongation_tpl(
        coarse_nnodes, coarse_rowptr, coarse_colidx, fine_node_map, vec_size,
        (double *)from, (double *)to, stream);
  }
  default: {
    SMESH_ERROR(
...
extern int cu_macrotet4_to_tet4_restriction(
    const ptrdiff_t coarse_nnodes,
    const count_t *const SMESH_RESTRICT coarse_rowptr,
    const idx_t *const SMESH_RESTRICT coarse_colidx,
    const idx_t *const SMESH_RESTRICT fine_node_map, const int vec_size,
...
static int cu_macrotet4_to_tet4_prolongation_element_based_tpl(
...
#ifdef SMESH_USE_OCCUPANCY_MAX_POTENTIAL
  {
    ...
    cudaOccupancyMaxPotentialBlockSize(
        &min_grid_size, &block_size,
        cu_macrotet4_to_tet4_prolongation_element_based_kernel<From, To>, 0, 0);
  }
#endif // SMESH_USE_OCCUPANCY_MAX_POTENTIAL
...
int cu_macrotet4_to_tet4_prolongation_element_based(
  ...
  case SMESH_FLOAT32: {
    return cu_macrotet4_to_tet4_prolongation_element_based_tpl(
        nelements, elements, vec_size, from_stride, (float *)from, to_stride,
        (float *)to, stream);
  }
  case SMESH_FLOAT64: {
    return cu_macrotet4_to_tet4_prolongation_element_based_tpl(
        nelements, elements, vec_size, from_stride, (double *)from, to_stride,
        (double *)to, stream);
  }
  default: {
    SMESH_ERROR(
...
__global__ void cu_macrotet4_to_tet4_restriction_element_based_kernel(
  ...
  for (ptrdiff_t e = blockIdx.x * blockDim.x + threadIdx.x; e < nelements;
    ...
    for (int v = 0; v < vec_size; v++) {
      ...
      atomicAdd(&to[ii0 * to_stride],
                from[ii4 * from_stride] * (0.5 / e2n_count[i4]));
      atomicAdd(&to[ii1 * to_stride],
...
static int cu_macrotet4_to_tet4_restriction_element_based_tpl(
...
#ifdef SMESH_USE_OCCUPANCY_MAX_POTENTIAL
  {
    ...
    cudaOccupancyMaxPotentialBlockSize(
        &min_grid_size, &block_size,
        cu_macrotet4_to_tet4_restriction_element_based_kernel<From, To>, 0, 0);
  }
#endif // SMESH_USE_OCCUPANCY_MAX_POTENTIAL
...
int cu_macrotet4_to_tet4_restriction_element_based(
  ...
  case SMESH_FLOAT32: {
    return cu_macrotet4_to_tet4_restriction_element_based_tpl(
        nelements, elements, element_to_node_incidence_count, vec_size,
        from_stride, (float *)from, to_stride, (float *)to, stream);
  }
  case SMESH_FLOAT64: {
    return cu_macrotet4_to_tet4_restriction_element_based_tpl(
        nelements, elements, element_to_node_incidence_count, vec_size,
        from_stride, (double *)from, to_stride, (double *)to, stream);
  }
  default: {
    SMESH_ERROR(
        "[Error] cu_tet4_to_macrotet4_prolongation_tpl: not implemented "

...
static int test_incidence_count() {
    auto elements = create_host_buffer<idx_t>(27, 2);
    ...
    auto count = create_host_buffer<uint16_t>(27 + 18);
    sshex8_element_node_incidence_count<idx_t>(2, 1, 2, elements->data(), count->data());

    for (ptrdiff_t i = 0; i < 18; i++) {
        SMESH_TEST_ASSERT(count->data()[i] == 1);
        SMESH_TEST_ASSERT(count->data()[27 + i] == 1);
    }

    for (ptrdiff_t i = 0; i < 9; i++) {
        SMESH_TEST_ASSERT(count->data()[i + 18] == 2);
    }
...
static int test_restrict_level2_to_level1() {
    ...
    auto elements = create_host_buffer<idx_t>(27, 1);
    ...
    auto count = create_host_buffer<uint16_t>(27);
    sshex8_element_node_incidence_count<idx_t>(2, 1, 1, elements->data(), count->data());

    SMESH_TEST_ASSERT(sshex8_restrict(1,                 // nelements,
                                      2,                 // from_level
                                      1,                 // from_level_stride
    ...
    for (ptrdiff_t i = 0; i < 8; i++) {
        SMESH_TEST_ASSERT(fabs(to->data()[i] - 3.375) < 1e-14);
    }
...
static int test_level1_to_level4() {
    ...
    SMESH_TEST_ASSERT(sshex8_prolongate(1,                      // nelements,
                                        1,                      // from_level
                                        1,                      // from_level_stride
    ...
    for (int zi = 0; zi < 5; zi++) {
        for (int yi = 0; yi < 5; yi++) {
            for (int xi = 0; xi < 5; xi++) {
                SMESH_TEST_ASSERT(fabs(to->data()[zi * 25 + yi * 5 + xi] - xi) < 1e-12);
            }
        }
    }
...
static int test_level1_to_level2() {
    auto from = create_host_buffer<real_t>(8);
    auto to   = create_host_buffer<real_t>(27);
    ...
    auto count_to = create_host_buffer<uint16_t>(27);
    sshex8_element_node_incidence_count<idx_t>(2, 1, 1, elements->data(), count_to->data());

    for (ptrdiff_t i = 0; i < 27; i++) {
        SMESH_TEST_ASSERT(count_to->data()[i] == 1);
    }

    auto count_from = create_host_buffer<uint16_t>(8);
    sshex8_element_node_incidence_count<idx_t>(1, 2, 1, elements->data(), count_from->data());

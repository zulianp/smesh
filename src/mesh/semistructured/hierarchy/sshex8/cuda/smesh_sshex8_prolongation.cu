...
__global__ void cu_sshex8_prolongate_kernel(
          ...
          for (int to_zi = start_zi; to_zi < end_zi; to_zi += 2) {
            for (int to_yi = start_yi; to_yi < end_yi; to_yi += 2) {
              for (int to_xi = start_xi; to_xi < end_xi; to_xi += 2) {
                ...
                const bool to_exists = e < nelements && off_to_zi <= to_level &&
                                       off_to_yi <= to_level &&
                                       off_to_xi <= to_level;
          ...
          for (int to_zi = start_zi; to_zi < end_zi; to_zi += 2) {
            for (int to_yi = start_yi; to_yi < end_yi; to_yi += 2) {
              for (int to_xi = start_xi; to_xi < end_xi; to_xi += 2) {
                ...
                //             out += Sx[rrvx * to_npoints] * Sy[rrvy *
                //             to_npoints] * Sz[rrvz * to_npoints] *
                //                    in[idx];
                //         }
                //     }
                // }
                ...
                for (int vz = 0; vz < 2; vz++) {
                  for (int vy = 0; vy < 2; vy++) {
                    for (int vx = 0; vx < 2; vx++) {
                      ...
                      out += Sx[vx * to_npoints] * Sy[vy * to_npoints] *
                             Sz[vz * to_npoints] * in[idx];
                    }
                  }
                }
...
int cu_sshex8_prolongate_tpl(
  ...
  if (stream) {
    ...
    cu_sshex8_prolongate_kernel<From, To>
        <<<n_blocks, block_size, shared_mem_size, s>>>(
            nelements, from_level, from_level_stride, from_elements, to_level,
            to_level_stride, to_elements,
#ifndef PROLONGATE_IN_KERNEL_BASIS_VERSION
            S.data,
#endif
            vec_size, from_type, from_stride, from, to_type, to_stride, to);
  } else {
    cu_sshex8_prolongate_kernel<From, To>
...
extern int cu_sshex8_prolongate(
    const ptrdiff_t nelements, const int from_level,
    const int from_level_stride, idx_t **const SMESH_RESTRICT from_elements,
    const int to_level, const int to_level_stride,
    idx_t **const SMESH_RESTRICT to_elements, const int vec_size,
  ...
  switch (from_type) {
  case SMESH_DEFAULT: {
    return cu_sshex8_prolongate_tpl<real_t, real_t>(
        nelements, from_level, from_level_stride, from_elements, to_level,
        to_level_stride, to_elements, vec_size, from_type, from_stride,
        (real_t *)from, to_type, to_stride, (real_t *)to, stream);
  }
  case SMESH_FLOAT32: {
    return cu_sshex8_prolongate_tpl<float, float>(
        nelements, from_level, from_level_stride, from_elements, to_level,
        to_level_stride, to_elements, vec_size, from_type, from_stride,
        (float *)from, to_type, to_stride, (float *)to, stream);
  }
  case SMESH_FLOAT64: {
    return cu_sshex8_prolongate_tpl<double, double>(
        nelements, from_level, from_level_stride, from_elements, to_level,
        to_level_stride, to_elements, vec_size, from_type, from_stride,
        (double *)from, to_type, to_stride, (double *)to, stream);
  }

  default: {
    SMESH_ERROR("[Error]  cu_sshex8_prolongate: not implemented for type "
                "%s "
                "(code %d)\n",

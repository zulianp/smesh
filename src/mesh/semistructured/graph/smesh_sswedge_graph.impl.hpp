#ifndef SMESH_SSWEDGE_GRAPH_IMPL_HPP
#define SMESH_SSWEDGE_GRAPH_IMPL_HPP
...
static int wedge6_edge_connectivity[6][3] = {{1, 2, 3}, {0, 2, 4}, {0, 1, 5}, {0, 4, 5}, {1, 3, 5}, {2, 3, 4}};

static const int sswedge_corner_xyz[6][3] = {{0, 0, 0}, {1, 0, 0}, {0, 1, 0}, {0, 0, 1}, {1, 0, 1}, {0, 1, 1}};

template <typename idx_t, typename count_t, typename element_idx_t>
static int wedge6_build_edge_graph_from_n2e(const ptrdiff_t                                         nelements,
    ...
    idx_t   *colidx = 0;
    static const int nnodesxelem = 6;
    rowptr[0]                    = 0;
    ...
    for (ptrdiff_t node = 0; node < nnodes; ++node) {
        ...
        for (count_t e = ebegin; e < eend; ++e) {
            ...
            for (int d = 0; d < 3; d++) {
                ...
                n2nbuff[nneighs++] = elems[wedge6_edge_connectivity[lidx][d]][eidx];
            }
        }
...
template <typename idx_t, typename count_t, typename element_idx_t>
static int wedge6_build_edge_graph(const ptrdiff_t                                         nelements,
                                   const ptrdiff_t                                         nnodes,
                                   const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                                   count_t                                               **out_rowptr,
                                   idx_t                                                 **out_colidx) {
    ...
    create_n2e<idx_t, count_t, element_idx_t>(nelements, nnodes, 6, elems, &n2eptr, &elindex);
    int err = wedge6_build_edge_graph_from_n2e(nelements, nnodes, elems, n2eptr, elindex, out_rowptr, out_colidx);
    SMESH_FREE(n2eptr);
    SMESH_FREE(elindex);
    return err;
}
...
template <typename idx_t, typename count_t, typename element_idx_t>
static int wedge6_build_edge_graph_from_multiblock_n2e(
...
#pragma omp parallel for
    for (ptrdiff_t node = 0; node < nnodes; ++node) {
        ...
        for (count_t e = ebegin; e < eend; ++e) {
            ...
            for (int d = 0; d < 3; d++) {
                SMESH_ASSERT(nneighs < 2048);
                n2nbuff[nneighs++] = block_elems[wedge6_edge_connectivity[lidx][d]][eidx];
            }
        }
        nneighs          = sort_and_unique(n2nbuff, nneighs);
        rowptr[node + 1] = nneighs;
...
static void sswedge_fill_lattice_coords(const int L, int **coords) {
...
static void sswedge_index_tri_face(const int                                               L,
    ...
    for (int t = 1; t <= L - 2; ++t) {
        for (int s = 1; s <= L - 1 - t; ++s) {
            ...
            const int pidx = sswedge_lidx(L, xi, yi, zi);
...
template <typename idx_t>
static void sswedge_index_quad_face(const int                                               L,
                                    const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT m_elements,
                                    const int *const                                        local_side_table,
                                    const int *const                                        lagr_to_proteus_corners,
                                    int *const *const                                       coords,
...
template <typename idx_t>
static void sswedge_index_owned_edges(const int                                               L,
                                      const ptrdiff_t                                         e,
                                      const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT m_elements,
    ...
    for (int d1 = 0; d1 < 6; d1++) {
        ...
        idx_t              g_neigh[3];
        for (int k = 0; k < 3; k++) {
            g_neigh[k] = nodes[wedge6_edge_connectivity[d1][k]];
        }
        idx_t offsets[3];
        ...
        for (int d2 = 0; d2 < 3; d2++) {
            ...
            const int lid2 = lagr_to_proteus_corners[wedge6_edge_connectivity[d1][d2]];
            int       P1[3], P2[3];
            for (int d = 0; d < 3; d++) {
                P1[d] = coords[d][lid1];
                P2[d] = coords[d][lid2];
            ...
            for (int t = 1; t < L; ++t) {
                ...
                const int lidx = sswedge_lidx(L, xi, yi, zi);
...
template <typename idx_t>
static void sswedge_index_volume(const int                                         L,
                                 const ptrdiff_t                                   e,
                                 const idx_t                                       index_base,
...
template <typename idx_t>
static int sswedge_assign_faces(const int                                               L,
                                const ptrdiff_t                                         m_nelements,
                                const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT m_elements,
                                const int *const                                        lagr_to_proteus_corners,
                                int *const *const                                       coords,
                                const element_idx_t *const                              adj_table,
                                idx_t                                                   index_base,
                                idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT       elements,
                                idx_t                                                  *n_unique_tri_out,
                                idx_t                                                  *n_unique_quad_out) {
    LocalSideTable lst;
    lst.fill(WEDGE6);
    const int nxf_tri  = sswedge_nx_tri_face(L);
    const int nxf_quad = sswedge_nx_quad_face(L);
    idx_t     n_tri    = 0;
    idx_t     n_quad   = 0;
    if (nxf_tri) {
        for (ptrdiff_t e = 0; e < m_nelements; e++) {
            for (int f = 3; f < 5; f++) {
                ...
                sswedge_index_tri_face(L, m_elements, lst.table, lagr_to_proteus_corners, coords, off, e, f, elements);
    ...
    if (nxf_quad) {
        for (ptrdiff_t e = 0; e < m_nelements; e++) {
            for (int f = 0; f < 3; f++) {
                ...
                sswedge_index_quad_face(L, m_elements, lst.table, lagr_to_proteus_corners, coords, off, e, f, elements);
                if (neigh != invalid_idx<element_idx_t>()) {
                    int neigh_f = 5;
                    for (int f2 = 0; f2 < 5; f2++) {
                        if (e == adj_table[neigh * 5 + f2]) {
    ...
    *n_unique_tri_out  = n_tri;
    *n_unique_quad_out = n_quad;
    return 0;
}

template <typename idx_t>
int sswedge_generate_elements(const int                                               L,
                              const ptrdiff_t                                         m_nelements,
    ...
    SMESH_ASSERT(L >= 1);
    int lagr_to_proteus_corners[6];
    for (int d = 0; d < 6; d++) {
        lagr_to_proteus_corners[d] =
                sswedge_lidx(L, sswedge_corner_xyz[d][0] * L, sswedge_corner_xyz[d][1] * L, sswedge_corner_xyz[d][2] * L);
    }
    int *coords[3];
    sswedge_fill_lattice_coords(L, coords);
    for (int d = 0; d < 6; d++) {
        for (ptrdiff_t e = 0; e < m_nelements; e++) {
    ...
    const int nxedge     = sswedge_nxedge(L);
    if (nxedge) {
        idx_t *rowptr{nullptr};
        idx_t *colidx{nullptr};
        wedge6_build_edge_graph<idx_t, idx_t, idx_t>(m_nelements, m_nnodes, m_elements, &rowptr, &colidx);
    ...
    create_element_adj_table(m_nelements, m_nnodes, WEDGE6, m_elements, &adj_table);
    idx_t n_tri = 0, n_quad = 0;
    sswedge_assign_faces(L,
                         m_nelements,
                         m_elements,
                         lagr_to_proteus_corners,
                         coords,
    ...
    index_base += n_tri * sswedge_nx_tri_face(L) + n_quad * sswedge_nx_quad_face(L);

    const int       nxvol          = sswedge_nxvol(L);
    const ptrdiff_t interior_start = index_base;
    if (nxvol) {
        for (ptrdiff_t e = 0; e < m_nelements; e++) {
            sswedge_index_volume(L, e, index_base, e, nxvol, elements);
        }
    }
    for (int d = 0; d < 3; d++) {
        SMESH_FREE(coords[d]);
...
template <typename idx_t>
int sswedge_generate_elements_blocks(
        const int                                                                     L,
        const ptrdiff_t                                                               n_blocks,
        const ptrdiff_t *const                                                        n_elements,
        const ptrdiff_t                                                               m_nnodes,
    ...
    if (nxedge) {
        ...
        idx_t *rowptr{nullptr};
        idx_t *colidx{nullptr};
        wedge6_build_edge_graph_from_multiblock_n2e<idx_t, idx_t, idx_t>(
    ...
    if (nxvol) {
        ...
        for (ptrdiff_t b = 0; b < n_blocks; ++b) {
            for (ptrdiff_t e = 0; e < n_elements[b]; e++) {
                sswedge_index_volume(L, e, index_base, e_off + e, nxvol, elements[b]);
            }
            e_off += n_elements[b];
        }
    }
...
template <typename idx_t>
int sswedge_hierarchical_renumbering(const int                                         L,
                                     const int                                         nlevels,
                                     int *const                                        levels,
...
#pragma omp parallel for
    ...
    idx_t next_id = 0;
    for (int c = 0; c < 6; c++) {
        const int v = sswedge_lidx(
                L, sswedge_corner_xyz[c][0] * L, sswedge_corner_xyz[c][1] * L, sswedge_corner_xyz[c][2] * L);
    ...
    for (int k = 1; k < nlevels; k++) {
        ...
        for (ptrdiff_t e = 0; e < nelements; e++) {
            for (int z = 0; z <= l; ++z) {
                for (int y = 0; y <= l; ++y) {
                    for (int x = 0; x <= l - y; ++x) {
                        const int   v   = sswedge_lidx(L, x * step_factor, y * step_factor, z * step_factor);
                        const idx_t idx = elements[v][e];
    ...
    for (int z = 0; z <= L; ++z) {
        for (int y = 0; y <= L; ++y) {
            for (int x = 0; x <= L - y; ++x) {
                const int v = sswedge_lidx(L, x, y, z);
                for (ptrdiff_t e = 0; e < nelements; e++) {
                    const idx_t idx = elements[v][e];
                    if (node_mapping[idx] == invalid_idx<idx_t>()) {
...
template <typename idx_t>
int sswedge_hierarchical_renumbering_blocks(
        const int                                                                     L,
        const int                                                                     nlevels,
        int *const                                                                    levels,
        const ptrdiff_t                                                               n_blocks,
...
template <typename idx_t>
static void sswedge_fill_tri_side(const int                                               L,
                                  const ptrdiff_t                                         e,
                                  const int                                               s,
    ...
    const int P0[3] = {sswedge_corner_xyz[c0][0] * L, sswedge_corner_xyz[c0][1] * L, sswedge_corner_xyz[c0][2] * L};
    const int P1[3] = {sswedge_corner_xyz[c1][0] * L, sswedge_corner_xyz[c1][1] * L, sswedge_corner_xyz[c1][2] * L};
    const int P2[3] = {sswedge_corner_xyz[c2][0] * L, sswedge_corner_xyz[c2][1] * L, sswedge_corner_xyz[c2][2] * L};
    int       lidx  = 0;
    for (int t = 0; t <= L; ++t) {
        for (int si = 0; si <= L - t; ++si) {
            ...
            sides[lidx++][i] = elems[sswedge_lidx(L, xi, yi, zi)][e];
        }
    }
}

template <typename idx_t>
static void sswedge_fill_quad_side(const int                                               L,
                                   const ptrdiff_t                                         e,
                                   const int                                               s,
                                   const LocalSideTable                                   &lst,
                                   const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
    ...
    const int P1[3] = {sswedge_corner_xyz[c1][0] * L, sswedge_corner_xyz[c1][1] * L, sswedge_corner_xyz[c1][2] * L};
    const int P2[3] = {sswedge_corner_xyz[c2][0] * L, sswedge_corner_xyz[c2][1] * L, sswedge_corner_xyz[c2][2] * L};
    const int P3[3] = {sswedge_corner_xyz[c3][0] * L, sswedge_corner_xyz[c3][1] * L, sswedge_corner_xyz[c3][2] * L};
    int       lidx  = 0;
    for (int t = 0; t <= L; ++t) {
        for (int sxy = 0; sxy <= L; ++sxy) {
            const int xi = (P0[0] * (L - sxy) * (L - t) + P1[0] * sxy * (L - t) + P2[0] * sxy * t + P3[0] * (L - sxy) * t) /
                           (L * L);
            const int yi = (P0[1] * (L - sxy) * (L - t) + P1[1] * sxy * (L - t) + P2[1] * sxy * t + P3[1] * (L - sxy) * t) /
                           (L * L);
            const int zi = (P0[2] * (L - sxy) * (L - t) + P1[2] * sxy * (L - t) + P2[2] * sxy * t + P3[2] * (L - sxy) * t) /
                           (L * L);
            sides[lidx++][i] = elems[sswedge_lidx(L, xi, yi, zi)][e];
        }
    }
...
int sswedge_extract_surface_from_sideset(const int                                               L,
                                         const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
...
#pragma omp parallel for
    for (ptrdiff_t i = 0; i < n_surf_elements; i++) {
        ...
        SMESH_ASSERT((s < 3) == quad);
        if (s < 3) {
            sswedge_fill_quad_side(L, e, s, lst, elems, i, sides);
        } else {
            sswedge_fill_tri_side(L, e, s, lst, elems, i, sides);
        }
    }
    return SMESH_SUCCESS;
}
...
template <typename idx_t, typename element_idx_t>
int sswedge_extract_nodeset_from_sideset(const int                                               L,
                                         const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                                         const ptrdiff_t                                         n_surf_elements,
                                         const element_idx_t *const SMESH_RESTRICT               parent_element,
                                         const i16 *const SMESH_RESTRICT                         side_idx,
    ...
    const int nnxs = (side_idx[0] < 3) ? ((L + 1) * (L + 1)) : sstet4_n_tri(L);
    const ptrdiff_t n = static_cast<ptrdiff_t>(nnxs) * n_surf_elements;
    auto            tmp = create_host_buffer<idx_t>(nnxs, n_surf_elements);
    if (sswedge_extract_surface_from_sideset(L, elems, n_surf_elements, parent_element, side_idx, tmp->data()) !=
        SMESH_SUCCESS) {

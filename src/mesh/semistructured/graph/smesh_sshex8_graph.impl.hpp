...
#pragma omp parallel for
    ...
    int hex8_build_edge_graph(const ptrdiff_t                                         nelements,
        ...
        create_n2e<idx_t, count_t, element_idx_t>(nelements, nnodes, 8, elems, &n2eptr, &elindex);
    ...
    int sshex8_skeleton_crs_graph(const int                                               L,
        ...
        count_t       *n2eptr;
        element_idx_t *elindex;
        create_n2e<idx_t, count_t, element_idx_t>(nelements, nnodes, 8, elements, &n2eptr, &elindex);
    ...
    ptrdiff_t nxe_max_node_id(const ptrdiff_t nelements, const int nxe, idx_t **const SMESH_RESTRICT elements) {
        ptrdiff_t ret = 0;
        for (int i = 0; i < nxe; i++) {
    ...
    int sshex8_generate_elements(const int                                               L,
        ...
        if (nxedge) {
            ...
            count_t *rowptr;
            idx_t   *colidx;
            hex8_build_edge_graph<idx_t, idx_t, count_t>(m_nelements, m_nnodes, m_elements, &rowptr, &colidx);
        ...
        if (nxf) {
            ...
            for (ptrdiff_t e = 0; e < m_nelements; e++) {
                for (int f = 0; f < 6; f++) {
                    ...
                    // continue
                    if (neigh_element != invalid_idx<element_idx_t>() && neigh_element < e) continue;

                    idx_t global_face_offset = index_base + n_unique_faces * nxf;
                    index_face(L, m_elements, lst.table, lagr_to_proteus_corners, coords, global_face_offset, e, f, elements);
...
#pragma omp parallel for collapse(3)
...
#endif
        element_idx_t *elindex = (element_idx_t *)SMESH_ALLOC(n2eptr[nnodes] * sizeof(element_idx_t));
    ...
    int sshex8_build_crs_graph_from_n2e(const int                                               L,
                                        const ptrdiff_t                                         nelements,
                                        const ptrdiff_t                                         nnodes,
                                        const idx_t *const SMESH_RESTRICT *const SMESH_RESTRICT elems,
                                        const count_t *const SMESH_RESTRICT                     n2eptr,
